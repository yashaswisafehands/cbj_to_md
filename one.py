...
# --- Configuration ---
INPUT_DIR = "cbj_input"
# OUTPUT_DIR = "cbj_output" # This is no longer used but can be kept if needed
...
def build_content_index(json_data: dict) -> dict:
    """Creates a fast lookup dictionary (index) from all content sections."""
...
def format_content_as_markdown(item: dict) -> str:
    """Formats a content item into a markdown string based on its structure."""
    markdown_parts = []
...
# CHANGE 1: Add 'base_filename' as a parameter to know where to save the files.
def process_modules(json_data: dict, content_index: dict, base_filename: str):
    """Iterates through the 'modules' section and assembles a single .md file for each."""
    ...
    # CHANGE 2: Create a specific sub-directory for this bundle's previews.
    bundle_preview_path = os.path.join(MODULE_PREVIEWS_DIR, base_filename)
    os.makedirs(bundle_preview_path, exist_ok=True)
    ...
    for module in json_data['modules']:
        ...
        # CHANGE 3: Save the file inside the newly created sub-directory.
        output_path = os.path.join(bundle_preview_path, output_filename)
...
def main():
    """Main function to orchestrate the bulk conversion process."""
    ...
    for filename in os.listdir(INPUT_DIR):
        if filename.endswith(".json"):
            ...
            # CHANGE 4: Pass the 'base_name' to the processing function.
            process_modules(data, index, base_name)
